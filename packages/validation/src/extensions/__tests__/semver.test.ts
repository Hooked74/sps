...
describe("validation/extensions/semver", () => {
  ...
  it("Версия не может содержать буквы", () => {
    expectJest(() => validation.attempt("a2.1.0", validation.semver())).toThrowError();
    expectJest(() => validation.attempt("2.1.a", validation.semver())).toThrowError();
  });

  it("Формат версионирования должен быть мажор.минор.патч", () => {
    expectJest(() => validation.attempt("2.1.", validation.semver())).toThrowError();
    expectJest(() => validation.attempt("2..1", validation.semver())).toThrowError();
    expectJest(() => validation.attempt("2.1", validation.semver())).toThrowError();
    expectJest(() => validation.attempt(".1", validation.semver())).toThrowError();
    expectJest(() => validation.attempt("1", validation.semver())).toThrowError();
  });

  it('Должны отработать проверки на сравнение "больше"', () => {
    validation.attempt("2.3.0", validation.semver().gt("2.2.2"));
    expectJest(() => validation.attempt("2.2.2", validation.semver().gt("2.2.2"))).toThrowError();
    expectJest(() => validation.attempt("2.2.0", validation.semver().gt("2.2.2"))).toThrowError();
  });
  ...
  it('Должны отработать проверки на сравнение "больше либо равно"', () => {
    ...
    expectJest(() => validation.attempt("2.2.0", validation.semver().gte("2.2.2"))).toThrowError();
  });

  it('Должны отработать проверки на сравнение "меньше"', () => {
    validation.attempt("2.1.0", validation.semver().lt("2.2.2"));
    expectJest(() => validation.attempt("2.2.2", validation.semver().lt("2.2.2"))).toThrowError();
    expectJest(() => validation.attempt("2.3.0", validation.semver().lt("2.2.2"))).toThrowError();
  });
  ...
  it('Должны отработать проверки на сравнение "меньше либо равно"', () => {
    ...
    expectJest(() => validation.attempt("2.3.0", validation.semver().lte("2.2.2"))).toThrowError();
  });

  it('Должны отработать проверки на сравнение "равно"', () => {
    validation.attempt("2.2.2", validation.semver().eq("2.2.2"));
    expectJest(() => validation.attempt("2.3.2", validation.semver().eq("2.2.2"))).toThrowError();
  });

  it('Должны отработать проверки на сравнение "не равно"', () => {
    validation.attempt("2.3.2", validation.semver().neq("2.2.2"));
    expectJest(() => validation.attempt("2.2.2", validation.semver().neq("2.2.2"))).toThrowError();
  });
  ...
  it("Должна отработать проверка, что версия выше диапазона", () => {
    ...
    expectJest(() =>
      validation.attempt("1.2.0", validation.semver().gtr(">=1.3.0"))
    ).toThrowError();
    expectJest(() =>
      validation.attempt("1.3.0", validation.semver().gtr(">=1.3.0"))
    ).toThrowError();
  });
  ...
  it("Должна отработать проверка, что версия ниже диапазона", () => {
    ...
    expectJest(() =>
      validation.attempt("1.3.0", validation.semver().ltr(">=1.3.0"))
    ).toThrowError();
    expectJest(() =>
      validation.attempt("1.4.0", validation.semver().ltr(">=1.3.0"))
    ).toThrowError();
  });
